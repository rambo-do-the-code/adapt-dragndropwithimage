  ...
  const DragndropwithimageView = QuestionView.extend({
    ...
    onDropOut: function (e, ui) {
      ...
      if (this.$currentDroppable && e.target === this.$currentDroppable[0]) {
        const name = this.$currentDraggable.attr("name");
        console.log(name);
        $(".user-answer")
          .find("img")
          .each(function (i) {
        ...
        if (name) {
          ...
          if (name && $droppable) {
            const item = items[$droppable.data("index")];
            console.log(item);
            console.log(item._userAnswer);
      ...
      if (!isShowAnswer) {
        ...
        _.each(
          ...
          function (question, index) {
            const $container = $(
              '<div class="user-answer" style="padding: 10px; display: block; position: absolute; top: 0; left: 0; width: 100%; display: flex; gap: 10px; flex-wrap: wrap; z-index: 20; visibility: hidden;"></div>'
            );
            const imageSources =
